...
type Props = Pick<
    IData_SnippetNews,
    'FAV' | 'DOM' | 'CNTR' | 'LANG' | 'AU' | 'CNTR_CODE' | 'URL'
>;

const { Text, Link } = Typography;

const NewsAuthoredInfo = ({
    // FAV,
    DOM,
    CNTR,
    LANG,
    ...
    const country = (emojiFlags as unknown as Record<string, CountryData>)[
        CNTR_CODE.toUpperCase()
    ];
    ...
    return (
        <div className={style.container}>
            ...
            <Text className={style.text}>
                {country.emoji && (
                    <div className={style.icon}>{country.emoji}</div>
                )}
                {CNTR}
            </Text>
