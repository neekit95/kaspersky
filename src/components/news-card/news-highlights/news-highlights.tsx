    ...
    const parseHighlights = (text: string): (string | JSX.Element)[] => {
        ...
        return parts.map((part, idx) => {
            if (part.startsWith('<kw>') && part.endsWith('</kw>')) {
                ...
                return (
                    <span key={idx} className={style.highlighted}>
                        {clean}
                    </span>
                );
            }
                ...
                <>
                    ...
                    {HIGHLIGHTS.length > 1 && (
                        ...
                        >
                            ...
                            {showAll ? (
                                <CaretUpFilled className={style.arrow} />
                            ) : (
                                <CaretDownFilled className={style.arrow} />
                            )}
                        </Text>
                    )}
                </>
            ) : (
                <Paragraph className={style.paragraph} ellipsis={{ rows: 3 }}>
                    {AB}
                </Paragraph>
            )}
